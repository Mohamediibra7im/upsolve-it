...
import {useRoadmapLevel} from "@/hooks/useRoadmap";
...
const LevelPage = () => {
  ...
  return (
    <div className="min-h-screen pb-20 pt-0 relative overflow-hidden bg-[linear-gradient(to_right,rgba(var(--primary),0.02)_1px,transparent_1px),linear-gradient(to_bottom,rgba(var(--primary),0.02)_1px,transparent_1px)] bg-[size:32px_32px]">
      ...
      <Orb className="w-[500px] h-[500px] bg-primary/6 dark:bg-primary/4 top-10 left-1/4 animate-pulse duration-[7s]" />
      <Orb className="w-[400px] h-[400px] bg-emerald-500/6 dark:bg-emerald-500/4 bottom-10 right-1/4 animate-pulse duration-[9s]" />
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10 max-w-7xl">
        ...
        >
          <div className="absolute inset-0 pointer-events-none">
            <div className="absolute -top-12 -right-12 h-64 w-64 rounded-full bg-primary/10 blur-[100px] opacity-70" />
            <div className="absolute bottom-0 left-10 h-48 w-48 rounded-full bg-emerald-500/5 blur-[80px] opacity-50" />
          </div>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {data.topics.map((topic, idx) => {
              ...
              const status = isLocked
                ...
                : isComplete
                  ? "Completed"
                  : topic.progress?.sheetUnlocked
                    ? "Practice Open"
                    : (topic.progress?.videoPct ?? 0) > 0
                      ? "Lecture Active"
                      : "Lecture Unwatched";
                  ...
                  {isLocked && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center rounded-3xl bg-background/95 dark:bg-background/90 text-center p-6 border border-border/80 dark:border-border/60 z-20">
                      <div className="rounded-2xl bg-muted border border-border p-3 mb-3 shadow-md">
                        <Lock className="h-5 w-5 text-muted-foreground/60 group-hover:scale-110 transition-transform duration-300" />
                      </div>
                      <h4 className="text-xs font-black uppercase tracking-[0.2em] text-foreground">
                        Mission Locked
                      </h4>
                      <p className="text-[10px] text-muted-foreground/75 mt-1 max-w-[200px] leading-relaxed font-medium">
                        Complete preceding practice sheets to establish connection.
                      </p>
                    </div>
                  )}
                    ...
                    <div className="space-y-1.5">
                      <div className="flex items-center justify-between text-[9px] font-black uppercase tracking-wider text-muted-foreground/50">
                        ...
                        <span className="text-foreground font-bold">
                          {Math.round(topic.progress?.videoPct ?? 0)}%
                        </span>
                      </div>
                      <div className="h-1.5 w-full rounded-full bg-background border border-border/40">
                        <div
                          ...
                          style={{width: `${topic.progress?.videoPct ?? 0}%`}}
                        />
                      </div>
                    </div>
